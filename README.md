...
7. Chat ビューでツール一覧に `argos` のツールが出ることを確認する
...
- [agents/reviewer.agent.md](agents/reviewer.agent.md): reviewer A の一次レビュー用
- [agents/examiner.agent.md](agents/examiner.agent.md): examiner 用
- [agents/rebuttal.agent.md](agents/rebuttal.agent.md): reviewer A の反論用
...
1 指摘ごとに review を分割するのではなく、A が 1 本のレビュー本文を保存し、その本文を複数の examiner session が吟味します。

### 基本方針

- A は、現在開いている PR について 1 回のレビュー本文を作成する
- レビュー本文の中に複数の指摘をまとめて含める
- 指摘がない場合は save_review を呼ばず、A のレビューだけで完結する
...
3. 次の 2 通りのどちらかで開始する
4. 手動開始: 任意のチャットウィンドウから start_session(review_id=..., reviewer="A") を呼ぶ
5. 自動開始: examiner agent に review_id だけを渡し、agent 側で start_session を実行させる
6. 追加の examiner が必要なら、同じ review_id でもう一度 start_session(review_id=..., reviewer="A") を呼ぶ
...
- A は自動でポーリングしない
- 利用者が Web UI の /sessions または /sessions/[sessionId] を見て、next_actor が A になったことを確認してから A に再度依頼する
- A は get_next_action(session_id=...) を確認し、自分の手番のときだけ get_session、get_session_messages を読んで `model_name` 付きで submit_message(session_id=..., agent="A", model_name=..., content=..., judgment=null) を呼ぶ
...
2. A 用チャットでは `reviewer` agent を選び、変更の目的や追加で重視したいレビュー観点を自由文で渡す
3. A に現在の PR をレビューさせる
4. 指摘がある場合だけ A が save_review を呼び、review_id を取得する
...
10. 利用者が next_actor を確認し、必要なときだけ A 用チャットで `rebuttal` agent を選び、session_id を渡して反論させる
...
- Web UI では保存された `model_name` を優先表示し、未保存の過去データは従来の Agent A / Examiner 表示にフォールバックする
